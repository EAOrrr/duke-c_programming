...
board_t * makeBoard(int w, int h, int numMines) {
  //WRITE ME!
  board_t *newb = (board_t*)malloc(sizeof(*newb));
  newb->width = w;
  newb->height = h;
  newb->totalMines = numMines;
  newb->board = malloc(h * sizeof(*newb->board));
  for(int i = 0; i < h; i++)
    newb->board[i] = malloc(w *sizeof(**newb->board));
  for(int y = 0; y < h; y++)
    for(int x =0; x < w; x++)
      newb->board[y][x] = UNKNOWN;
